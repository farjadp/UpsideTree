...
import {
  Upload, Folder, Image as ImageIcon, FileText, Search, Grid, List,
  Trash2, Sparkles, X, FolderPlus, ExternalLink
} from "lucide-react";
...
export function MediaLibrary({ page, folder: selectedFolder, fileType: fileTypeFilter, search }: MediaLibraryProps) {
  ...
  // Filters and page live in the URL; any filter change returns to page 1.
  const navigate = (next: { folder?: string; type?: string; q?: string }) => {
    ...
    const href = buildPageHref(
      ...
      {
        ...filterParams,
        ...(next.folder !== undefined ? { folder: next.folder !== "all" ? next.folder : undefined } : {}),
        ...(next.type !== undefined ? { type: next.type !== "all" ? next.type : undefined } : {}),
        q: q || undefined,
      },
      1
    );
  ...
  useEffect(() => {
    if (searchQuery.trim() === lastPushedSearch.current) return;
    const timer = setTimeout(() => navigate({ q: searchQuery }), 350);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchQuery]);

  // Paginated at the database: only the current page of rows is fetched.
  const fetchMedia = useCallback(async () => {
    try {
      ...
      if (fileTypeFilter !== "all") query = query.eq("file_type", fileTypeFilter);

      // or() is comma/paren-delimited; strip those so a term can't inject a clause.
      const term = search.trim().replace(/[,()*%]/g, " ").trim();
      if (term) {
        query = query.or(`original_name.ilike.*${term}*,alt_text_en.ilike.*${term}*,alt_text_fa.ilike.*${term}*`);
      }

      const { data, error, count } = await query;

      if (error && error.code === "PGRST103" && page > 1) {
        router.replace(buildPageHref("/admin/media", filterParams, 1));
        return;
      }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, selectedFolder, fileTypeFilter, search]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {mediaList.length === 0 ? (
            ...
            <div
              className={`${viewMode === "grid" ? "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4" : "space-y-2"} transition-opacity ${
          ...
          <div className="rounded-2xl bg-slate-900/50 border border-white/10">
            <AdminPagination
              page={page}
              total={total}
